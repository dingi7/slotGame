...
//         payout = payouts['7'];
//     } else if (combination.includes('000')) {
//         payout = payouts['000'];
//     }
...
// // Simulate 100,000 plays with a bet amount of 10 units
// const plays = 500000;
// const betAmount = 10;
...
function play(betAmount: number): [number, [GameSymbol, GameSymbol, GameSymbol]] {
    const result = spin();
    const payout = calculatePayout(result, betAmount);
    console.log(`Spin result: ${result.join(' ')} - Payout: ${payout}`);
    return [payout, result];
