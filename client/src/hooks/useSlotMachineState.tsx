...
export const useSlotMachineState = () => {
  ...
  const [isDoubleWinModalOpenable, setIsDoubleWinModalOpenable] =
  ...
  //   useEffect(() => {
  //     if (!desiredNums || desiredNums.length === 0) {
  //       return;
  //     }
  ...
  //       // Remove desired symbols from their current positions
  //       for (let row = 0; row < rowsCount; row++) {
  //         const rowSymbols = desiredNums[row];
  //         rowSymbols.forEach((symbol: number, colIndex: number) => {
  //           newState[colIndex].assets = newState[colIndex].assets.filter(
            ...
            prevStates.map((column: ReelStateType, reelIndex: number) => {
              ...
              if (
                ...
                if (
                  ...
                  // Ensure the reel stops at the last occurrence of the desired symbol
                  const lastOccurrence = newAssets
                    .slice(-rowsCount)
                    .findIndex(
                      (asset) => asset.value === desiredNums[0][reelIndex]
                    );

                  if (
                    lastOccurrence === -1 ||
                    lastOccurrence === rowsCount - 1
                  ) {
