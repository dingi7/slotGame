...
export const useSlotMachineState = () => {
        ...
        setColumnStates((prevState) => {
            ...
            for (let row = 0; row < rowsCount; row++) {
                ...
                rowSymbols.forEach((symbol: number, colIndex: number) => {
                    newState[colIndex].assets = newState[
                        colIndex
                    ].assets.filter((asset) => asset.value !== symbol);
                });
            }
    ...
    const [reelIconsMoved, setReelIcons] = useState(
        ...
        if (spinningReels.some((spinning) => spinning)) {
            const interval = setInterval(() => {
                ...
                setColumnStates(
                    (prevStates: Reels) =>
                        prevStates.map(
                            (column: ReelStateType, reelIndex: number) => {
